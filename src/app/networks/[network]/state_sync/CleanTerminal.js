    ...
    <Box sx={{ position: "relative", width: { sm: 600, md: 1000 } }}>
      ...
      >
        <Tooltip title="Copy to Clipboard">
          ...
          >
            {copied ? <CheckCircleOutlineIcon sx={{ fontSize: 20 }} /> : <ContentCopyIcon sx={{ fontSize: 20 }}/>}
          </IconButton>
        </Tooltip>
        <Typography>
          sed -i "/\[statesync\]/, /^enable =/ s/=.*/= false/;\
        </Typography>
        <Typography>/^rpc_servers =/ s|=.*|= \"\"|;\ </Typography>
        <Typography>/^trust_height =/ s/=.*/= 0/;\ </Typography>
        <Typography>
          /^trust_hash =/ s/=.*/= \"\"/" $HOME/.andromedad/config/config.toml{" "}
        </Typography>
      </Box>
    </Box>
  );
